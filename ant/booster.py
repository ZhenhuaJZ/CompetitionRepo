...
feats_selet_param = {
        ...
        "train_path" : "", #[train_heatmap , train_mode_fill, train]
        "test_path" : "", #[test_a_heatmap, test_a_mode_fill, test_a]
...
param = {
        ...
        "test_path" : "data/test_b.csv", #test_a_heatmap, test_a_mode_fill, test_a,
...
#***********************model & score path ***********************************************#
#log
log = "/log/day{}/".format(now.day)
#model save path#
model_path = log + "model/"
#where to save the figure & answer & hParams
score_path = log + "/score/{}_{}_{}h/".format(now.month, now.day, now.hour, now.minute)
...
def select_features(_train,_label,_test):
    ...
    with open(info_file + "importance_features.txt" , "w") as _log:
        for f in range(len(feats_seleted)):
            _log.write(str(f + 1) + "." +  " feature " +  str(feats_seleted[f]) + "  " + str(round(f_score[f] , 3)) + "\n")
        _log.write(">"*30 + "Selected Features" + "<"*30 + "\n")

        for f in range(len(_feats_seleted)):
            _log.write(str(f + 1) + "." +  " feature " +  str(_feats_seleted[f]) + "  " + str(round(_f_score[f] , 3)) + "\n")
